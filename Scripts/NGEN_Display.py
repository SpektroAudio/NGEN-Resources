...
class App:
    ...
    def on_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_s:
                ...
                screenshot_location = (
                    sys.path[0]
                    + "/output/NGEN_Screenshot_"
                    + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                    + ".jpeg"
                )
                pygame.image.save(self.display, screenshot_location)
                newConsole.print(
                    "[purple]Saved screenshot to: {}".format(screenshot_location)
                )
    ...
    def on_render(self):
        ...
        try:
            pygame.draw.rect(
                self.display, (0, 0, 0), pygame.Rect(0, 0, self.width, self.height)
            )
                ...
                for col in range(len(last_buffer[0])):
                    ...
                    if value == 1:
                        ...
                        pygame.draw.rect(
                            self.display,
                            (255, 255, 255),
                            pygame.Rect(x, y, self.scale, self.scale),
                        )
            if SAVE_VIDEO:
                pygame.draw.ellipse(
                    self.display,
                    (255, 0, 0),
                    pygame.Rect(10, 10, 10 * self.scale, 10 * self.scale),
                )
...
def exportMP4():
    ...
    os.system(
        'ffmpeg -r 20 -i "{}img_%04d.png" -vcodec mpeg4 -y "{}movie.mp4"'.format(
...
def printBuffer(buffer):
    ...
    for i in range(len(buffer)):
        ...
        if i < len(last_buffer):
            last_buffer[i] = row
        else:
            last_buffer.append(row)
...
def getSerialPort():
    ...
    results = subprocess.Popen(
        ["grep", "usbmodem"], stdin=results.stdout, stdout=subprocess.PIPE
    )
